...
Manager::Manager() 
{
	...
	comment_id = 1;
	network_money = 0;
	user.push_back(new Customer(ADMIN, "", ADMIN, ADMIN, 1, 1));
		...
		try
		{
			...
			if(lines_seperated.size() > 0)
			{
				if(lines_seperated[FIRST_ELEMENT] == POST && lines_seperated.size() >= 2)
				{
					...
					else if(lines_seperated[SECOND_ELEMENT] == LOGOUT)
						logout(username, password);
					else if(lines_seperated[SECOND_ELEMENT] == PUTFILMS && lines_seperated[THIRD_ELEMENT] == QUESTION_MARK)
						edit_film(username, lines_seperated);	
					else if(lines_seperated[SECOND_ELEMENT] == DELETEFILMS && lines_seperated[THIRD_ELEMENT] == QUESTION_MARK)
						delete_films(username, lines_seperated);
					else if(lines_seperated[SECOND_ELEMENT] == DELETECOMMENTS && lines_seperated[THIRD_ELEMENT] == QUESTION_MARK)
				...
				else if(lines_seperated[FIRST_ELEMENT] == GET)
				{
					...
					else if(lines_seperated[SECOND_ELEMENT] == MONEY && lines_seperated.size() == 2)
						get_money(username);
					else
						throw Not_Found();
				}
...
void Manager::logout(std::string &username, std::string &password)
{
	if(username == "")
		throw Bad_Request();
	username = "";
...
void Manager::get_money(std::string username)
{
	if(user_is_publisher(username))
	{
		for(int i = 0; i < publisher.size(); i++)
		{
			if(publisher[i].return_username() == username)
				std::cout<< publisher[i].how_much_money() << std::endl;
		}
	}
	else
	{
		for(int i = 0; i < user.size(); i++)
		{
			...
			{
				if(user[i]->get_RUP(ROLE) == ADMIN)
					std::cout<< network_money / 5 << std::endl;
				else
					std::cout<<user[i]->how_much_money()<<std::endl;
			}
		}
	}
}
...
void Manager::delete_films(std::string username, std::vector<std::string> lines_seperated)
{
	...
	for(int i = 0; i < user.size(); i++)
		if(user[i]->has_bought_film(film_id))
			user[i]->put_flag_one(film_id);
	for(int i = 0; i < publisher.size(); i++)
		if(publisher[i].has_bought_film(film_id))
			publisher[i].put_flag_one(film_id);
	for(int i = 0; i < matrix.size(); i++)
...
void Manager::recommend(std::string username, int film_id)
{
	...
	search_for_reccommend(films_copy, username);
}
...
void Manager::search_for_reccommend(std::vector<Film> films_copy, std::string username)
{
	int line_counter = 0;
	int counter = 0;
	for(int i = 0; i < films_copy.size(); i++)
	{
		...
		if(user_is_publisher(username))
		{
			for(int j = 0; j < publisher.size(); j++)
			{
				if(publisher[j].return_username() == username)
				{
					...
					{
						std::cout<<line_counter + 1<<". "<<films_copy[i].return_PLPYI(FILM_ID)<<SEPERATOR<<films_copy[i].return_NDS(NAME);
						std::cout<<SEPERATOR<<films_copy[i].return_PLPYI(LENGTH)<<SEPERATOR<<films_copy[i].return_NDS(DIRECTOR)<<std::endl;
						line_counter++;
						break;
					}
				}
		...
		else
		{
			for(int j = 0; j < user.size(); j++)
			{
				if(user[j]->get_RUP(USERNAME) == username)
				{
					if(user[j]->has_bought_film(films_copy[i].return_PLPYI(FILM_ID)) && films_copy[i].return_PLPYI(FILM_ID) != film_id)
						break;
					else
					{
						std::cout<<line_counter + 1<<". "<<films_copy[i].return_PLPYI(FILM_ID)<<SEPERATOR<<films_copy[i].return_NDS(NAME);
